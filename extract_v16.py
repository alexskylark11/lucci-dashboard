"""Extract clean depletion data from Lucci report (16).xlsx (through 7/10/26)."""
import pandas as pd
import re
...
months_by_width = {
    ...
    32: (['Nov','Dec','Jan','Feb','Mar','Apr','May','Jun','Jul'], False, 2),
...
for m in ['Mar','Apr','May','Jun','Jul']:
    if m in months:
        agg3[f'{m}_Cases'] = 'sum'
top = rest.groupby(['RetailAcct','Chain','State','City','TradeChannel']).agg(agg3).reset_index().sort_values('YTD_Cases', ascending=False).head(16)
